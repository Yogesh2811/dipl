...
class Key
    ...
    def initialize(str)
        ...
        (1..10).each { |i| @key << get_next_rk(@key[i-1],i.rcon) }
    end
    ...
    def get_next_rk(prev, rcon)
        k = Array.new_aes()
        k << prev.col(3).rotate(1).map{|block| block.sbox}.xor(prev.col(0)).xor(rcon)
        (1..3).each { |i|
            k << prev.col(i).xor(k[i-1])
        }
        return State.new(k.transpose.is_aes!)
    end

    def print_nice(base)
        @key.each{ |k| k.print_nice(base) }
    end
...
class State
    ...
    def to_s()
        s = ""
        each_block { |block| s << block.to_s_two(16)}
        return s
    end
end
...
class AES
    ...
    def self.mixColumnsInv(state)
        ...
        state.each_column { |col, pos|
            a0, a1, a2, a3 = col[0], col[1], col[2], col[3]
            mx[pos] = [
                a0.g14 ^ a3.g9 ^ a2.g13 ^ a1.g11,
                a1.g14 ^ a0.g9 ^ a3.g13 ^ a2.g11,
                a2.g14 ^ a1.g9 ^ a0.g13 ^ a3.g11,
                a3.g14 ^ a2.g9 ^ a1.g13 ^ a0.g11
            ]
        }
        return State.new( mx.transpose.is_aes! )
    end
    ...
    def self.cipher(text, key)
        ...
        arr.each{ |x| 
            x = x.ljust(32,"0")
            cipher_text << cipher_state(State.new(x))
        }
        return cipher_text
    end
    ...
    def self.decipher(ciphered_text, key)
        text = ""
        @@key = Key.new(key)
        arr = ciphered_text.scan(/.{32}/)
        arr.each{ |x| 
            x = x.ljust(32,"0")
            text << decipher_state(State.new(x))
        }
        return text
    end
end
