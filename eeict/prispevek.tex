...
ensures communication with both endpoints and is running in its own thread. It contains
buffer pools for incomming and outgoing data to ensure maximal level of parallelization
in each layer of application. Pointers for input and output buffers are passed for further
packet processing where are extracted information such as header from the packet, copied 
data from the memory to OpenCL data structures and serial implementation of AES key schedule.
Parallel processing of payload 
Graph visualizes only packet encoding, however, decoding is executed in similar manner.
...
\begin{figure}[h!]
\centering
\includegraphics[width=13cm]{paral_scheme.pdf}
