...
One of the essential metrics for measuring VoIP gateway's 
...
reasonig behind their selection. Even though the focus of the thesis is primarily
research of available contemporary methods there were many restrictions.
The requirements of this chapter arise from currently used implementation and 
hardware limitation of the gateway.

Finally chapters \ref{chapter:results} and \ref{chapter:conclusion} sumarize
...
which the implemetation could be based on and as the field of VoIP communication
has been known for decent amount of time, there are currently couple of advised
design patterns, from which the particular implementation for this thesis
stands on four -- mediator pattern, singleton pattern, factory method pattern 
\cite{design-patterns} and protocol stack pattern \cite{protocol-stack}. None of
these design patterns could be thought as contribution of the thesis as they all
belong to common public knowledge and their examination was not the main topic 
of the  research. However, their explanation is provided in order to make the
rest of the chapter more comprehendable.
...
behavorial pattern called mediator, which is named after the way it alters the
running behavior. The pattern consits of following participants:
...
\url{http://en.wikipedia.org/wiki/Mediator\_pattern}
...
class must prevent from instantiation of multiple instances,  it must be globaly 
available from well known access point
\end{itemize}
\url{http://en.wikipedia.org/wiki/Singleton\_pattern}
...
in RFC-3711 \cite{rfc3711} in at least CRT mode that provides protection of transfered
...
\item \textbf{transcoding module} -- due to vaious reasons, endpoints may not be
...
SRTP stack should be encapsulated to minimize overloading data transferes with
...
An advanced techniques like \textbf{jitter buffer} may improve overal quality
...
\begin{itemize}
\item \textbf{Network Thread} -- the incomming and outgoing data are captured
...
\item \textbf{Stack Thread} -- the interaction and selectiong atributes for
the processing thread is taken care in the stack thread as well as interface
for higher layers of the aplication using the SRTP stack.
...
the stack design. The SRTP layers as shown in the scheme \ref{scheme} have following
functions:

\begin{itemize}
\item \textbf{Network Layer} --
\item \textbf{Packet Processing Layer} --
\item \textbf{Payload Encoding} --
\end{itemize}

\subsection{Serial Processing}
For better understanding of improvement this thesis is provided with reference
serial implementation which design will be analyzed as well.  

\subsection{Massive Parallel Processing}

\subsection{Persistent Thread Processing}
