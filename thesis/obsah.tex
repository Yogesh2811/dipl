...
from network and their processed variant.It includes the buffers for incomming and 
...
processing SRTP header in incomming packet and another pointer to \texttt{dst}
...
costly allocations during sessions, auxilliary buffers are statically allocated
...
column of previous round key applicated on substitution box
...
hardware as HiPath4000 softgate is equiped. During the tests, only SIP gateway
...
physical atributes of used materials, the focus drifts more on the design
...
for examination of alternative kernel design named persisten thread. Trading off
portion of parallelization for higher level of control over kernel execution
and limiting OpenCL computational overhead ratio proven itself as valid aproach
...
successfull to the greater degree lowering the delay to one third for maximal
number of concurrent channels which is set to the gateway as 120.

At first, comparison of implementated algorithms efectivity was measured on
